...
namespace Meas3D.Tool
{
    ...
    {
        private Tools3DManager _manager;

        public Action<ETool3DType> OnAddTool;
        ...
        public void Init(Tools3DManager manager)
        {
            _manager = manager;
            ...
            foreach (Tool3DBaseModel tool in _manager.ToolsList)
            {
                AddTool(tool);
        ...
        private void CloseSetupPanel(Tool3DBaseModel tool)
        {
            Panel_Setup.Visible = false;
            Panel_Setup.Controls.Clear();
        ...
        private void AddTool(Tool3DBaseModel tool)
        {
            //注意控件的Button_Copy,Button_Setup,Button_Delete的Anchor要设置成left，否则显示大小不对
            tool.NormalView.Size = new Size(Panel_Tool_List.Width - 25, tool.NormalView.Height);
            Panel_Tool_List.Controls.Add(tool.NormalView);
        }

        private void RemoveTool(Tool3DBaseModel tool)
        {
